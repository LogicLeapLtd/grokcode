...
/// Owns the GrokCode licensing lifecycle: the 7-day no-card trial, online
...
final class LicenseManager: ObservableObject {
    ...
    /// True only when the hard paywall should cover the app.
    ...
    private func storedTrialStart() -> Date? {
        if let raw = KeychainStore.get(account: Account.trialStart) ?? defaults.string(forKey: DefaultsKey.trialStart),
    ...
    private func storedLastSeen() -> Date? {
        if let raw = KeychainStore.get(account: Account.lastSeen) ?? defaults.string(forKey: DefaultsKey.lastSeen) {
    ...
    /// Issue a JSON POST to a Lemon Squeezy license endpoint with the required
    /// headers and decode the (shared-shape) response.
    ///
    ...
    private func storedLicenseKey() -> String? {
        let value = KeychainStore.get(account: Account.licenseKey) ?? defaults.string(forKey: DefaultsKey.licenseKey)
        guard let value, !value.isEmpty else { return nil }
        return value
    }

    private func storedInstanceId() -> String? {
        let value = KeychainStore.get(account: Account.instanceId) ?? defaults.string(forKey: DefaultsKey.instanceId)
        guard let value, !value.isEmpty else { return nil }
        return value
    }

    private func storedLastValidated() -> Date? {
        if let raw = KeychainStore.get(account: Account.lastValidated) ?? defaults.string(forKey: DefaultsKey.lastValidated) {
            return isoFormatter.date(from: raw)
        }
        return nil
    }
    ...
    private func instanceName() -> String {
        Host.current().localizedName ?? "GrokCode (\(ProcessInfo.processInfo.hostName))"
    }
